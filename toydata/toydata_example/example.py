...
def unmapped_info(df):
    ...
    return pd.DataFrame({"chr": chromosome[:rows], "location": location}, index=df.columns)
unmapped_info=unmapped_info(u_df)

#interaction mapping
interact_unmapped, interact_mapped = it.map_interaction(interact, unmapped_info, mapped_info, neighborhood=20)
...
#sparse with:
#continuous values
s_c_isn = it.sparse_isn(u_df, interact_unmapped, interact_mapped, metric="pearson", pool="avg")
#discrete values
s_d_isn = it.sparse_isn(m_df, interact_unmapped=None, interact_mapped, metric="pearson")
